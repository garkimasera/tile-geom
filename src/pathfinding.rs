use crate::{Array2d, Coords, Direction};
...
impl<F: Fn(Coords) -> bool> PathFinding<F> {
    ...
    pub fn route(&self, start: Coords, end: Coords) -> Option<Vec<Coords>> {
        ...
        while let Some((step, dir, pos)) = q.pop_back() {
            ...
            for &dir in Direction::EIGHT_DIRS.iter() {
                let next_pos = pos + dir.as_coords();
